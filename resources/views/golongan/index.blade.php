                ...
                <tbody>
                    @foreach($golongan as $index => $item)
                        <tr>
                            ...
                            <td>{{ $item->nama_Gol }}</td>
                            <td>
                                <a href="{{ route('golongan.edit', $item->id_Gol) }}" class="btn btn-sm btn-warning me-2">Edit</a>
                                <form action="{{ route('golongan.destroy', $item->id_Gol) }}" method="POST" style="display:inline-block">
                                    @csrf
                                    @method('DELETE')
